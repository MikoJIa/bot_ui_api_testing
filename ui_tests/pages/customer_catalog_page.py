...
class CustomerCatalogPage(BaseClass):
    ...
    @allure.feature("Названия страницы")
    @allure.description("Проверка url-а на соответствие названию")
    def check_catalog_page(self, text):
        with allure.step("Получение url-а страницы"):
            url = self.get_current_url()
            word = re.search(r"\w+$", url)
        assert word.group() == text

    @allure.feature("Все товары на странице")
    @allure.description("Проверка на соответсвие ожидаемому результату количество товаров на странице")
    def check_all_products_to_page(self, number):
        with allure.step("Получение всех товаров на странице"):
    ...
    @allure.feature("Кнопка отправки запроса")
    @allure.description("Проверка, работает ли кнопка - 'Отправить запрос'")
    ...
    @allure.feature("Форма отравки запроса")
    @allure.description("Проверяем форму для заполнения запроса")
    ...
    @allure.description("Проверка формы заказа")
    def check_form_to_order_soft(self, title_soft, quantity_soft, my_name, phone, describe, text):
        with allure.step("Прокрутка страницы до кнопки с оформления заказа"):
            self.link_to_soft()
            ...
            with allure.step("Заполнение формы для заказа"):
